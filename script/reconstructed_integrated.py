...
def add_residential_popup_layers(
    ...
    for _, row in residential_result_wgs84.iterrows():
        ...
        medical_text_normal = f"一般情境醫療可及性分數：{med_score_normal:.3f}"
        shelter_text_normal = f"一般情境避難可及性分數：{shel_score_normal:.3f}"
        medical_text_scenario = f"{scenario_name} 情境醫療可及性分數：{med_score_scenario:.3f}"
        shelter_text_scenario = f"{scenario_name} 情境避難可及性分數：{shel_score_scenario:.3f}"

        if pd.notna(row.get("nearest_medical_id", np.nan)):
            medical_row = medical_lookup.loc[row["nearest_medical_id"]]
            medical_text_normal = (
                f"一般情境最近醫療目的地 ID：{row['nearest_medical_id']}<br>"
        ...
        if pd.notna(row.get(medical_id_col, np.nan)):
            ...
            medical_text_scenario = (
                f"{scenario_name} 情境最近醫療目的地 ID：{row[medical_id_col]}<br>"
...
def build_scenario_map(
    ...
    for _, row in shelters_result_wgs84.iterrows():
        ...
        popup_lines.append(f"經度: {row.geometry.x:.6f}")
        popup_lines.append(f"緯度: {row.geometry.y:.6f}")
...
for scenario_name in scenario_names:
    ...
    m.save(html_path)
    print("已輸出互動地圖：", html_path.resolve())
