...
#%% 14. 繪製整合版互動式地圖：淹水 + 路網 + 情境道路 + 住宅最近醫療/避難所資訊
...
def find_nearest_destination_in_scenario(
    ...
    for origin_node in origins_result[origin_node_col]:
        try:
            ...
            nearest_dest_ids.append(
                dest_node_to_id.get(nearest_dest_node, nearest_dest_node)
            )
            nearest_distances.append(calc_route_cost(G, route, "length"))
...
print(f"{scenario_name} 情境道路淹水屬性計算完成")
print("道路總數：", len(edges_scenario_3826))
print("不可通行道路數：", int(edges_scenario_3826[blocked_col].sum()))
print("可通行道路數：", int((~edges_scenario_3826[blocked_col]).sum()))

print("\n淹水深度統計：")
print(edges_scenario_3826[depth_col].describe())

print("\n情境通行時間統計（僅可通行道路）：")
print(edges_scenario_3826.loc[~edges_scenario_3826[blocked_col], scenario_time_col].describe())


# ============================================================
# 5. 用情境路網重算住宅到醫療/避難所通行時間
# ============================================================
residential_scenario_3826 = residential_origins_3826.copy()
...
print(f"\n{scenario_name} 情境住宅可及性計算完成")
print("\n無法到達醫療場所的住宅點數量：")
print(residential_scenario_3826[medical_time_col].isna().sum())
print("\n無法到達避難所的住宅點數量：")
print(residential_scenario_3826[shelter_time_col].isna().sum())

print(f"\n{scenario_name} 情境到醫療場所通行時間統計：")
print(residential_scenario_3826[medical_time_col].describe())

print(f"\n{scenario_name} 情境到避難所通行時間統計：")
print(residential_scenario_3826[shelter_time_col].describe())


# ============================================================
# 5-1. 計算一般情境與淹水情境可及性分數
# ============================================================
residential_scenario_3826["medical_access_score_normal"] = residential_scenario_3826["time_to_medical_min"].apply(
    lambda x: calc_accessibility_score(x, max_accept_time_medical)
)
residential_scenario_3826["shelter_access_score_normal"] = residential_scenario_3826["time_to_shelter_min"].apply(
    lambda x: calc_accessibility_score(x, max_accept_time_shelter)
)

residential_scenario_3826[f"medical_access_score_{scenario_name}"] = residential_scenario_3826[medical_time_col].apply(
    lambda x: calc_accessibility_score(x, max_accept_time_medical)
)
residential_scenario_3826[f"shelter_access_score_{scenario_name}"] = residential_scenario_3826[shelter_time_col].apply(
    lambda x: calc_accessibility_score(x, max_accept_time_shelter)
)
...
print("\n一般情境醫療可及性分數統計：")
print(residential_scenario_3826["medical_access_score_normal"].describe())

print("\n一般情境避難可及性分數統計：")
print(residential_scenario_3826["shelter_access_score_normal"].describe())

print(f"\n{scenario_name} 情境醫療可及性分數統計：")
print(residential_scenario_3826[medical_score_scenario_col].describe())

print(f"\n{scenario_name} 情境避難可及性分數統計：")
print(residential_scenario_3826[shelter_score_scenario_col].describe())


# ============================================================
# 6. 轉成 WGS84，給 Folium 使用
# ============================================================
study_area_wgs84 = study_area_3826.to_crs(wgs84)
road_boundary_wgs84 = road_boundary_3826.to_crs(wgs84)
...
# ============================================================
# 15. 住宅點：加入一般情境 + 淹水情境資訊
# ============================================================
for idx, row in residential_result_wgs84.iterrows():
    res_lon = row.geometry.x
    res_lat = row.geometry.y

    medical_text_normal = "一般情境最近醫療：無法到達"
    shelter_text_normal = "一般情境最近避難所：無法到達"
    medical_text_scenario = f"{scenario_name} 情境最近醫療：無法到達"
    shelter_text_scenario = f"{scenario_name} 情境最近避難所：無法到達"
    ...
    if pd.notna(row.get("nearest_medical_id", np.nan)):
        ...
        medical_text_normal = (
            ...
            f"最快時間: {row['time_to_medical_min']:.2f} 分鐘<br>"
            f"醫療可及性分數: {med_score_normal:.3f}"
        )
    else:
        medical_text_normal = (
            "一般情境最近醫療：無法到達<br>"
    ...
    if pd.notna(row.get("nearest_shelter_id", np.nan)):
        ...
        shelter_text_normal = (
            ...
            f"最快時間: {row['time_to_shelter_min']:.2f} 分鐘<br>"
            f"避難可及性分數: {shel_score_normal:.3f}"
        )
    else:
        shelter_text_normal = (
            "一般情境最近避難所：無法到達<br>"
    ...
    if pd.notna(row.get(medical_id_col, np.nan)):
        ...
        medical_text_scenario = (
            ...
            f"醫療經度: {medical_row_s.geometry.x:.6f}<br>"
            f"醫療緯度: {medical_row_s.geometry.y:.6f}<br>"
            f"最快時間: {row[medical_time_col]:.2f} 分鐘<br>"
            f"醫療可及性分數: {med_score_scenario:.3f}"
        )
    else:
        medical_text_scenario = (
    ...
    if pd.notna(row.get(shelter_id_col, np.nan)):
        ...
        shelter_text_scenario = (
            ...
            f"避難所經度: {shelter_row_s.geometry.x:.6f}<br>"
            f"避難所緯度: {shelter_row_s.geometry.y:.6f}<br>"
            f"最快時間: {row[shelter_time_col]:.2f} 分鐘<br>"
            f"避難可及性分數: {shel_score_scenario:.3f}"
        )
    else:
        shelter_text_scenario = (
...
#             f"醫療經度: {medical_row_s.geometry.x:.6f}<br>"
#             f"醫療緯度: {medical_row_s.geometry.y:.6f}<br>"
#             f"最快時間: {row[medical_time_col]:.2f} 分鐘"
#         )
...
#             f"避難所經度: {shelter_row_s.geometry.x:.6f}<br>"
#             f"避難所緯度: {shelter_row_s.geometry.y:.6f}<br>"
#             f"最快時間: {row[shelter_time_col]:.2f} 分鐘"
#         )
